...
A tecnologia de eye tracking (ET) permite o rastreamento ocular, o que possibilita sua aplicação em diversas áreas das ciências, como saúde, psicologia e até mesmo marketing. O aparelho realiza a gravação do comportamento ocular, ou seja, identificando a localização do olhar, a duração
de tempo, a dilatação da pupila e o trajeto do olhar. 
A importância desta técnica se deve ao fato de que o olhar de um indivíduo está diretamente relacionado com a sua atenção, o que faz com que seja possível compreender, em parte, o processo
cognitivo de um usuário. 

A distância entre o reflexo que sai da pupila e o reflexo da luz observado na córnea é o que permite identificar a localização do olhar de uma pessoa, esse ponto onde
a pessoa fixa o olhar é chamado de “fixação”. A fixação geralmente dura entre 100 e 500 ms (milissegundos). O trajeto que se produz entre duas fixações é chamado de sacada (saccade).
...
```{r, results='hide'}
#Identificamos a diferença entre um elemento menos o #anterior, para isso usamos o comando "diff" para ter 
diff(dados[,1])
```
...
* 30 segundos em repouso seguidos de 30s de tarefa (repetindo ciclo)
...
Calculando as condições (para aula fizemos de uma forma manual para ficar mais rápido). O recomendado é reproduzir as linhas acima num laço e identificar as condições e intervalos.
A grosso modo o intervalo seria desta forma:
...
Para visualização dos dados, analisamos a curva média da pupila nesses períodos.
